...
public class DoublingArr<T> extends GrowableArr<T>{
	private T[][] outerArr;
	public DoublingArr(int initialArrSize) throws Exception {
		if(initialArrSize == 0) {
			throw new Exception("Size shouldn't be 0.");
		}
		outerArr = (T[][]) new Object[1][1];
		/*The convertIndex's while loop will grow the arr to
		 * have the index of the initialArrSize as an index.*/
		convertIndex(initialArrSize - 1, "");
	}
